  ...
  if (isRecording) {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        // Dodaj wykrywanie ciszy - DODANE TUTAJ
        this.setupSilenceDetection(stream, mediaRecorder);
        ...
        setTimeout(() => {
          if (mediaRecorder && mediaRecorder.state !== "inactive") {
            ...
            .catch((error) => {
              ...
              this.addMessage(
                "assistant",
                "Nie udało się uzyskać dostępu do mikrofonu. Upewnij się, że masz włączony mikrofon i zezwoliłeś na dostęp do niego.",
              );
      ...
      if (window && messagesContainer) {
        ...
        textBtn.addEventListener("click", () => {
          ...
          this.shadowRoot.querySelector(".input-container").style.display =
            "flex";
          if (this.voiceModal) {
    ...
    makeDraggable() {
      ...
      let offsetX, offsetY;
      let hasMoved = false; // Track if window has been moved during drag
      ...
      function onDrag(e) {
        ...
        // Check if window has moved more than a few pixels
        const moveThreshold = 5;
        if (
          Math.abs(boundedX - chatWindow.getBoundingClientRect().left) >
            moveThreshold ||
          Math.abs(boundedY - chatWindow.getBoundingClientRect().top) >
            moveThreshold
        ) {
          hasMoved = true;
        }
      ...
      function stopDrag(e) {
        ...
        // If the window was moved, prevent click events
        if (hasMoved) {
          e.preventDefault();
          e.stopPropagation();
        }
...
/**
 * Setup silence detection for auto-stopping recording
...
setupSilenceDetection(stream, mediaRecorder) {
    ...
    const silenceThreshold = 10; // Próg dla ciszy
    const silenceDuration = 2000; // 2sekundy ciszy, aby zatrzymać nagrywanie
    ...
    sendAudioToServer(audioBlob) {
      ...
      formData.append("audio", audioBlob, "recording.wav"); // Dodano nazwę pliku
      ...
      // Log debugging info
      if (this.config.debug) {
        console.log("BC Assistant: Sending audio to server", {
...
.then((data) => {
    ...
    // Handle response
    if (data.success) {
        // Sprawdź, czy to jest specjalna akcja
        if (data.data && data.data.special_action === 'call_reception') {
            // Dodaj informację o łączeniu
            this.addMessage("assistant", data.data.message);
            
            // Wykonaj połączenie telefoniczne
            if (data.data.phone_number) {
                setTimeout(() => {
                    // Użyj natywnego API telefonu do wykonania połączenia
                    window.location.href = `tel:${data.data.phone_number}`;
                }, 2000); // 2 sekundy opóźnienia, aby użytkownik zobaczył komunikat
            }
            
            // Zapisz thread_id jeśli istnieje
            if (data.data.thread_id) {
                this.state.threadId = data.data.thread_id;
                localStorage.setItem("bc_assistant_thread_id", this.state.threadId);
            }
        } else {
            // Standardowa obsługa odpowiedzi - istniejący kod
            ...
            if (data.data && data.data.message) {
                ...
                if (data.data.speech) {
                    this.playAudioResponse(data.data.speech);
                }
            }
        ...
        .catch((error) => {
          ...
          this.addMessage(
            "assistant",
            `Przepraszam, wystąpił błąd połączenia: ${error.message}. Spróbuj ponownie później.`,
          );

          // Log error
          console.error("BC Assistant Voice Error:", error);
        });
    }
    ...
    render() {
      ...
      this.shadowRoot.innerHTML = `
                <style>
					...
					:host([theme="dark"]) .audio-indicator {
						color: #aaa;
					}
...
@media (max-width: 767px) {
    :host {
        ...
        z-index: 2147483647 !important; /* Maksymalna wartość z-index */
    ...
    async sendMessage() {
      ...
      try {
        ...
        this.addMessage(
          "assistant",
          "Przepraszam, wystąpił błąd połączenia. Spróbuj ponownie później.",
        );
    ...
    playAudioResponse(base64Audio) {
      try {
        ...
        if (lastMessage && lastMessage.classList.contains("assistant")) {
          ...
          audioIndicator.innerHTML = "🔊 Odtwarzanie...";
          lastMessage.appendChild(audioIndicator);

          // Remove indicator when audio finishes
          audio.onended = () => {
            if (audioIndicator) {
        ...
        audio.play().catch((err) => {
          ...
          const indicators =
            this.shadowRoot.querySelectorAll(".audio-indicator");
