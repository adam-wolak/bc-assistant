...
if (isset($_POST['bc_assistant_save_settings']) && current_user_can('manage_options')) {
    ...
    if (isset($_POST['bc_assistant_nonce']) && wp_verify_nonce($_POST['bc_assistant_nonce'], 'bc_assistant_settings')) {
        ...
        // Zapisz nowe ustawienia pozycji
        if (isset($_POST['bc_assistant_bubble_position'])) {
            $bubble_position = sanitize_text_field($_POST['bc_assistant_bubble_position']);
            BC_Assistant_Config::set('bubble_position', $bubble_position);
        }
        
        if (isset($_POST['bc_assistant_auto_position'])) {
            $auto_position = 1;
        } else {
            $auto_position = 0;
        }
        BC_Assistant_Config::set('auto_position', $auto_position);
        
        if (isset($_POST['bc_assistant_theme'])) {
            $theme = sanitize_text_field($_POST['bc_assistant_theme']);
